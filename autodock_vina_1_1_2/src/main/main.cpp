...
void writeAtomCoordsLigand(model& m){

	vecv atom_coordinates = m.get_ligand_coords();
	for (std::vector<vec>::iterator it = atom_coordinates.begin() ; it != atom_coordinates.end(); ++it){
		std::cout << " Maria model (final model): " <<(*it).data[0] << "\n";				 
	}
}
...
void do_randomization(model& m,
	...
	std::cout << " Maria do_randomization: " << "\n"; 
...
void refine_structure(model& m, const precalculate& prec, non_cache& nc, output_type& out, const vec& cap, sz max_steps = 1000) {
	std::cout << "Maria: Refining structure: " << "\n"; 
...
void do_search(model& m, const boost::optional<model>& ref, const scoring_function& sf, const precalculate& prec, const igrid& ig, const precalculate& prec_widened, const igrid& ig_widened, non_cache& nc, // nc.slope is changed
	...
	std::cout << " Maria do_search " << "\n"; 			
	conf_size s = m.get_size();
	conf c = m.get_initial_conf();
	fl e = max_fl;
	const vec authentic_v(1000, 1000, 1000);
	if(score_only) {
		std::cout << "Maria: score_only " << score_only << "\n"; 
		
		fl intramolecular_energy = m.eval_intramolecular(prec, authentic_v, c);
		std::cout << "Maria: Intramolecular energy " << intramolecular_energy << "\n"; //Maria: fitness (the lowest the energy, the better the ligand's conformation)
		...
		std::cout << "Maria: local_only and NO score_only" << local_only << "\n"; 
		output_type out(c, e);
		doing(verbosity, "Performing local search", log);
		refine_structure(m, prec, nc, out, authentic_v, par.mc.ssd_par.evals);
		done(verbosity, log);
		fl intramolecular_energy = m.eval_intramolecular(prec, authentic_v, out.c);// Evaluation
		std::cout << " Intramolecular energy " << intramolecular_energy << "\n"; 
		...
		std::cout << "Maria: NO local_only NO score_only 1 " << local_only << "\n"; //AQUÍ
		...
		// Pintar vector out_cont

		std::cout << "Maria: NO local_only NO score_only 2 " << local_only << "\n"; //AQUI
		
		doing(verbosity, "Refining results", log);
		VINA_FOR_IN(i, out_cont)
			refine_structure(m, prec, nc, out_cont[i], authentic_v, par.mc.ssd_par.evals); //Maria: where Autodock Vina does the refinement

		if(!out_cont.empty()) {
			out_cont.sort();
			const fl best_mode_intramolecular_energy = m.eval_intramolecular(prec, authentic_v, out_cont[0].c);
			//std::cout << "Maria: best_mode_intramolecular_energy " << best_mode_intramolecular_energy << "\n"; 
...
void main_procedure(model& m, const boost::optional<model>& ref, // m is non-const (FIXME?)
	...
	//std::cout << " Grid size x : " << gd[0].begin << " " << gd[0].end <<"\n"; 		
	//std::cout << " Grid size y : " << gd[1].begin << " " << gd[1].end << "\n"; 
	//std::cout << " Grid size z : " << gd[2].begin << " " << gd[2].end <<"\n"; 		
		
	//std::cout << " Grid size y : " << gd[2] << "\n"; 		


	parallel_mc par;
	sz heuristic = m.num_movable_atoms() + 10 * m.get_size().num_degrees_of_freedom(); //Maria: Size of the heuristic
	std::cout << " Heuristic size: " << heuristic << "\n"; 		
	...
	if(randomize_only) {
		...
		if(no_cache) {
			std::cout << " Maria no do_randomization and no cache (no randomize only): " << "\n"; 
			...
			std::cout << " Maria no do_randomization and cache: " << "\n"; 		  
...
#################################################################\n";
	...
	try {
		...
		std::cout << "Starting to debugging...";
		...
		std::cout << "Maria: ligand_name " << " "<< ligand_name << "\n";
		std::cout << "Maria: rigid_name " << " " << rigid_name << "\n";
		std::cout << "Maria: flex_name " << flex_name << "\n";
		std::cout << "Maria: center_x " << " " << center_x << "\n";
		std::cout << "Maria: center_y " << " " << center_y << "\n";
		std::cout << "Maria: center_z " << " " << center_z << "\n";
		...
		std::cout << " Maria: weight_gauss1 : " <<  weight_gauss1 << "\n";
		std::cout << " Maria: weight_gauss2 : " <<  weight_gauss2 << "\n" ;
		std::cout << " Maria: weight_gauss2 : " <<  weight_gauss2 << "\n";
		std::cout << " Maria: weight_repulsion : " <<  weight_repulsion << "\n";
		std::cout << " Maria: weight_hydrophobic : " <<  weight_hydrophobic << "\n";
		std::cout << " Maria: weight_hydrogen : " <<  weight_hydrogen << "\n";
		std::cout << " Maria: weight_rot : " <<  weight_rot << "\n";
		...
		if(receptor_needed) {
			if(vm.count("receptor") <= 0) {
				std::cerr << "Missing receptor.\n" << "\nCorrect usage:\n" << desc_simple << '\n'; // Maria: missing receptor -> program is closed
				return 1;
			}
		}
		if(vm.count("ligand") <= 0) {
			std::cerr << "Missing ligand.\n" << "\nCorrect usage:\n" << desc_simple << '\n'; // Maria: missing ligand -> program is closed
			return 1;
		}
		if(cpu < 1) 
			cpu = 1;
		...
		if(search_box_needed) { 
			...
			if(size_x <= 0 || size_y <= 0 || size_z <= 0)
				throw usage_error("Search space dimensions should be positive"); // Maria: Checking dimenstions are positive or negative.
		}

		log << cite_message << '\n';

		if(search_box_needed && size_x * size_y * size_z > 27e3) {
			log << "WARNING: The search space volume > 27000 Angstrom^3 (See FAQ)\n"; // Maria: Checking dimenstions of the gird used to limit an interaction.
		}
		...
		if(search_box_needed) { 
			...
			std::cout << "Maria size_x: " << size_x << "\n";
			std::cout << "Maria size_y: " << size_y << "\n";	
			std::cout <<  "Maria size_z: " << size_z << "\n";	
