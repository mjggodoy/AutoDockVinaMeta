 ...
 * @param problem Problem to solve
 */
gGA::gGA(Problem *problem, string fitFile) : Algorithm(problem, fitFile) {
} // gGA
...
//  cout << "Comienza la inicializacion de la poblacion con size " << populationSize << endl;
  ...
  out << evaluations << "\t" << population->best(comparator)->getObjective(0) << endl ;
  int interval = maxEvaluations / 5;
...
//  cout << "gGA: Maximo de evaluaciones: " << maxEvaluations << endl;
  ...
  while (evaluations < maxEvaluations) {
    ...
    out << evaluations << "\t" << population->get(0)->getObjective(0) << endl ;
    if (evaluations % interval == 0) {
      std::stringstream sstmFit;
      sstmFit << fitFile_ << "_" << evaluations;
      string popFile = sstmFit.str();
      population->printObjectivesToFile(popFile);
    }
  }
