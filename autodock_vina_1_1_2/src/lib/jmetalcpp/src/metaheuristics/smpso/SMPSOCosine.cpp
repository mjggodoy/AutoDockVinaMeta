...
#include <SMPSOCosine.h>


/*
 * This class implements the SMPSO algorithm using the CosineDistanceArchive.
 */
 ...
 */
SMPSOCosine::SMPSOCosine(Problem *problem) : Algorithm(problem) {
  r1Max_ = 1.0;
  r1Min_ = 0.0;
  r2Max_ = 1.0;
  r2Min_ = 0.0;
...
void SMPSOCosine::initParams() {
  ...
  referencePoint = new NadirPoint(problem_->getNumberOfObjectives());
  leaders_ = new CosineDistanceArchive(archiveSize_, problem_->getNumberOfObjectives(), referencePoint);

  // Create comparators for dominance and crowding distance
  dominance_ = new DominanceComparator();
  cosineDistanceComparator_ = new CosineDistanceComparator();
...
void SMPSOCosine::deleteParams() {
  ...
  delete [] speed_;
  delete dominance_;
  delete cosineDistanceComparator_;
  delete distance_;
  delete [] deltaMax_;
  delete [] deltaMin_;
  delete particles_;
...
// Adaptive inertia
double SMPSOCosine::inertiaWeight(int iter, int miter, double wma, double wmin) {
  return wma; // - (((wma-wmin)*(double)iter)/(double)miter);
} // inertiaWeight


// constriction coefficient (M. Clerc)
double SMPSOCosine::constrictionCoefficient(double c1, double c2) {
  double rho = c1 + c2;
  //rho = 1.0 ;
  if (rho <= 4) {
    return 1.0;
...
// velocity bounds
double SMPSOCosine::velocityConstriction(double v, double * deltaMax,
 ...
 */
void SMPSOCosine::computeSpeed(int iter, int miter) {
  ...
  for (int i = 0; i < swarmSize_; i++) {
    ...
    if (cosineDistanceComparator_->compare(one, two) < 1) {
 ...
 */
void SMPSOCosine::computeNewPositions() {
  for (int i = 0; i < swarmSize_; i++) {
    XReal * particle = new XReal(particles_->get(i)) ;
    for (int var = 0; var < particle->getNumberOfDecisionVariables(); var++) {
 ...
 */
void SMPSOCosine::mopsoMutation(int actualIteration, int totalIterations) {
...
/**
 * Runs of the SMPSOCosine algorithm.
 * @return a <code>SolutionSet</code> that is a set of non dominated solutions
 * as a result of the algorithm execution
 */
SolutionSet * SMPSOCosine::execute() {
