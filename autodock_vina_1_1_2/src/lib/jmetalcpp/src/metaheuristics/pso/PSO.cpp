 ...
 * @param problem Problem to solve
 */
PSO::PSO(Problem *problem, string fitFile) : Algorithm(problem, fitFile) {
 ...
 * @param problem Problem to solve
 */
PSO::PSO(Problem *problem, string fitFile, map<string, void *> parameters) : Algorithm(problem, fitFile) {
  ...
  if (parameters["C1"]!=NULL)
      C1 = *(double *)parameters["C1"];
  if (parameters["C2"]!=NULL)
      C2 = *(double *)parameters["C2"];
  
  cout << "PSO: Using C1=" << C1 << " and C2=" << C2 << endl;

  comparator_ = new ObjectiveComparator(0); // Single objective comparator
  map<string, void *> operatorParameters; // Operator parameters

  operatorParameters["comparator"] = comparator_;
  findBestSolution_ = new BestSolutionSelection(operatorParameters);
...
SolutionSet * PSO::execute() {
  ...
  std::fstream out(fitFile_.c_str(), std::ios::out | std::ios::app);
  int interval = maxIterations_ / 5;
  ...
  out << evaluations_ << "\t" << globalBest_->getObjective(0) << endl ;
  ...
  while (iteration_ < maxIterations_) {
    ...
    out << evaluations_ << "\t" << globalBest_->getObjective(0) << endl ;
    if (iteration_ % interval == 0) {
      std::stringstream sstmFit;
      sstmFit << fitFile_ << "_" << evaluations_;
      string popFile = sstmFit.str();
      particles_->printObjectivesToFile(popFile);
    }
