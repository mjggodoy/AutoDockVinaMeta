...
/**
 * Returns the cosene distance between two Solution objects in objective space
 * using a reference point.
 * @param solutionI The first <code>Solution</code>.
 * @param solutionJ The second <code>Solution</code>.
 * @return the distance between solutions in objective space.
 */
double Distance::cosineDistanceBetweenSolutionsInObjectiveSpace
    (Solution * solution1, Solution * solution2, ReferencePoint * referencePoint) {
  //double diff;    //Auxiliar var
  double sum = 0.0;
  ...
  double result = sum /
      (sumOfDistancesToIdealPoint(solution1, referencePoint)
      * sumOfDistancesToIdealPoint(solution2, referencePoint)) ;

  return result ;
} // cosineDistanceBetweenSolutionsInObjectiveSpace

double Distance::sumOfDistancesToIdealPoint(Solution * solution, ReferencePoint * referencePoint) {
  double sum = 0.0 ;
  for (int i = 0 ; i < solution->getNumberOfObjectives(); i++) {
    sum += pow(solution->getObjective(i) - referencePoint->getObjective(i), 2.0) ;
  }
  return sqrt(sum) ;
}
...
/**
 * Assigns cosine distances to all solutions in a <code>SolutionSet</code>.
 * @param solutionSet The <code>SolutionSet</code>.
 * @param nObjs Number of objectives.
 */
void Distance::cosineDistanceAssignment(SolutionSet * solutionSet, int nObjs, ReferencePoint * referencePoint) {
  int size = solutionSet->size();
  ...
  //Use a new SolutionSet to prevent altering original solutionSet
  SolutionSet * front = new SolutionSet(size);
  for (int i = 0; i < size; i++){
    front->add(solutionSet->get(i));
    referencePoint->update(solutionSet->get(i));
  }
...
//  double distance;
  ...
  for (int i = 0; i<nObjs; i++) {
    ...
    //Set the cosine distance (using the crowding distance variable)
    front->get(0)->setCrowdingDistance(-std::numeric_limits<double>::max());
    //front->get(size-1)->setCrowdingDistance(std::numeric_limits<double>::max());
  }
  ...
  // INICIALIZAMOS DISTANCE

  for (int i = 1; i < size-1; i++) {
    
    double d1 = cosineDistanceBetweenSolutionsInObjectiveSpace(front->get(i-1), front->get(i), referencePoint);
    double d2 = cosineDistanceBetweenSolutionsInObjectiveSpace(front->get(i), front->get(i+1),referencePoint);
    double sumCosines = d1 + d2;
    front->get(i)->setCrowdingDistance(sumCosines);
